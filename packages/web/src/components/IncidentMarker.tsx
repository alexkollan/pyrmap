...
// The flame stays a fixed neutral color so it reads clearly against the white disc no matter
// what the pin body's age color is.
const FLAME_COLOR = '#4b5563';
...
function pinSvg(color: string): string {
  ...
  <path d="M13.3 7.6c.9 2 2.7 3 2.7 5.4a3 3 0 01-6 0c0-.5.1-.9.3-1.4-1.2 1.1-1.9 2.5-1.9 4a3.9 3.9 0 007.8 0c0-3.7-1.9-5.4-2.9-8z" fill="${FLAME_COLOR}"/>
</svg>`;
}

/**
 * A fire reported by the Fire Service's own X account, geocoded from free Greek text — not a
 * satellite detection, so it deliberately doesn't reuse FireMarker's shapes (same exemption from
 * dev-plan §8.2 as WindLayer's divIcon, see docs/DECISIONS.md 2026-07-20). The pin body fades
 * red -> orange -> green -> blue -> grey over 12h from publishedAt, same gradient as the
