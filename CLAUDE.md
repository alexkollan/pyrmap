...
- Remote is `origin` on GitHub, branch `main`, linear history. No branches, no force-push, no history rewriting.
- **The repo is currently PUBLIC, deliberately.** It was private originally; made public 2026-07-20 to route around a confirmed-open Portainer CE bug (GitHub issue #10340, present as of 2.43.0 STS) where editing/redeploying an existing Git-sourced stack unreliably clears/mishandles the saved Git PAT, unrelated to token scope or correctness — public repos need no git auth at all, so the bug can't fire. No secret has ever been committed to this repo's history (verified by grepping full history for every live credential value in use) — the visibility change is safe for that reason, not just convenient. **Do not make it private again** without the user explicitly accepting that tradeoff; it will likely reintroduce the Portainer redeploy failures.
...
- **A new env var is not "done" until it's in all four places.** Any task that adds an env var must add it to `config.ts` (reader), `.env` (real local value), `.env.example` (documented placeholder), and `docker-compose.yml`'s `environment:` block — cross-check the four name sets match before considering the task complete. Missing the last one is silent: the app just runs with the feature off, with no error anywhere. This exact miss (auth vars never added to `docker-compose.yml`) cost a full session of misdiagnosing it as a Portainer bug — see `docs/DECISIONS.md` 2026-07-20.
- If the user pastes a live credential/token into chat, do not echo it back; tell them to revoke/rotate it immediately at its issuing service, and treat it as compromised regardless of anything said afterward unless they explicitly confirm a freshly-generated replacement is in use.
